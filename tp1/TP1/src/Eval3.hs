...
update v n (s, trc) = (M.insert v n s, trc)

-- Agrega una traza dada al estado
-- Completar la definición
addTrace :: String -> State -> State
addTrace str (s, trc) = (s, trc ++ " -- " ++ str) 
...
stepComm r@(Repeat c0 b) s = 
  Right (Seq c0 (IfThenElse b Skip r) :!: s)

stepComm (Let var ei) s =
  case evalExp ei s of
    Right (a :!: b)  -> 
      let f s' = addTrace ("Let " ++ var ++ " " ++ (show a)) s'
      in Right (Skip :!: f (update var a s)) 
    Left err -> Left err 
...
evalExp (EAssgn v ei) s = 
  case (evalExp ei s) of
    Right (a :!: b)  -> 
      let f s' = addTrace ("Let " ++ v ++ " " ++ (show a)) s'
      in Right (a :!: f (update v a s)) 
    Left err -> Left err
