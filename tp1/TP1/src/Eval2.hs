...
evalExp cons s =
  case cons of 
    (Plus e0 e1)  -> biAxu (+) e0 e1 s
    (Minus e0 e1) -> biAxu (-) e0 e1 s
    (Times e0 e1) -> biAxu (*) e0 e1 s
    (Div e0 e1)   -> divAux e0 e1 s
    (ESeq e0 e1)  -> biAxu (\ x y -> y) e0 e1 s
    (UMinus e)    -> uAux (\x -> -x) e s
    ----------------
    (Eq e0 e1)    -> biAxu (==) e0 e1 s
    (NEq e0 e1)   -> biAxu (/=) e0 e1 s
    (Lt e0 e1)    -> biAxu (<) e0 e1 s
    (Gt e0 e1)    -> biAxu (>) e0 e1 s
    (Or e0 e1)    -> biAxu (||) e0 e1 s
    (And e0 e1)   -> biAxu (&&) e0 e1 s
    (Not e)       -> uAux (not) e s

biAxu :: (a -> a -> b) -> Exp a -> Exp a -> State -> Either Error (Pair b State)
biAxu op e e' s = 
  case (evalExp e s) of
    Right (a1 :!: b1) ->  
